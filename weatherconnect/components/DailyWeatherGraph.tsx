...
interface DailyWeatherGraphProps {
  ...
  setSelectedIndicators: React.Dispatch<
...
export function DailyWeatherGraph({
  data,
  selectedIndicators,
  setSelectedIndicators,
}: DailyWeatherGraphProps) {
  if (!data) {
  ...
  if (isShortTerm) {
    const graphData = data.kma.shortTerm!.map((entry) => ({
      ...
      windSpeed: parseFloat(entry['풍속'].replace('m/s', '').trim() || '0'),
    }));

    const maxTemp = Math.max(
      ...graphData.map((entry) => entry.temperature || -Infinity)
    ).toFixed(1);
    ...
    const avgHumidity = Math.round(
      graphData.reduce(
        (sum, entry) =>
          sum +
          (parseFloat(
            data.kma.shortTerm![graphData.indexOf(entry)]["습도"].replace(
              "%",
              ""
            )
          ) || 0),
    ...
    const avgWindSpeed = (
      graphData.reduce((sum, entry) => sum + (entry.windSpeed || 0), 0) /
      graphData.length
    ).toFixed(1);

    const summary = (
      <div className="mt-4 text-gray-700 flex flex-col items-center">
        <p><span className="font-bold">최고 기온:</span> {maxTemp}°C</p>
        <p><span className="font-bold">최저 기온:</span> {minTemp}°C</p>
        <p><span className="font-bold">평균 습도:</span> {avgHumidity}%</p>
        <p><span className="font-bold">평균 풍속:</span> {avgWindSpeed}m/s</p>
      </div>
      
    );

    return (
      <Card className="w-full max-w-4xl mt-6 hover:shadow-lg hover:scale-105 transition-transform duration-300">
        <CardHeader>
        ...
        <CardContent>
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={() =>
                  setSelectedIndicators((prev) => ({
                    ...prev,
                    precipProbability: !prev.precipProbability,
                  }))
                }
              />
          ...
          <div className="transform hover:scale-110 active:scale-100 transition-transform duration-300">
            <ResponsiveContainer width="100%" height={400}>
              ...
              >
                ...
                <Legend
                  ...
                  height={40}
                  iconSize={16}
                  wrapperStyle={{ fontSize: 14, paddingBottom: 20 }}
                />
